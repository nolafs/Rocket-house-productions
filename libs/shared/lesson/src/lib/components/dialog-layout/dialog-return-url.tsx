...
export function DialogReturnUrl({ title, children, classNames }: DialogLayoutProps) {
  const router = useRouter();
  const sp = useSearchParams();
  const closeTo = sp.get('returnTo') || undefined;

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') closeTo ? router.push(closeTo) : router.back();
    };
      ...
      <div className="relative pb-3 text-center">
        ...
        <div
          ...
          <header
            ...
            <div className={'absolute right-5 top-5'} onClick={() => router.back()}>
...
export default DialogReturnUrl;
